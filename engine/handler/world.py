...
class DefaultTile:
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            raise ValueError(
                f"Data item `{key}` cannot be serialized because value: `{value}` is not serializable\nError as : {e}"
            )
...
class Chunk:
    ...
    def __init__(
        ...
        self._pixel_coords = (
            ...
            * self._chunk_tile_dimensions[0]
            * singleton.DEFAULT_TILE_WIDTH,
            self._chunk_position[1]
            * self._chunk_tile_dimensions[1]
            * singleton.DEFAULT_TILE_HEIGHT,
        )
        ...
        self._tile_pixel_area = (
            (
                singleton.DEFAULT_TILE_WIDTH
                if not tile_dimensions[0]
                else tile_dimensions[0]
            ),
            (
                singleton.DEFAULT_TILE_HEIGHT
                if not tile_dimensions[1]
                else tile_dimensions[1]
            ),
        )

        self._chunk_rect = pygame.Rect(
            self._pixel_coords,
        ...
        # create tile storage
        self._tiles = [
            [None for j in range(self._chunk_tile_dimensions[0])]
                ...
                if singleton.DEBUG or singleton.EDITOR_DEBUG:
                    pygame.draw.rect(
                        ...
                        pygame.Rect(
                            ...
                            - _camera.position
                            + self._chunk_offset,
                            self._sprite_cacher[tile._sprite_path].get_size(),
                        ),
                        1,
                    )
    ...
    def set_tile_at(self, position: tuple, tile: DefaultTile):
        ...
        tile[CHUNK_TILE_PARENT_POSITION_KEY] = self.get_chunk_hash_str(
    ...
    @classmethod
    def generate_chunk_rect_given_chunk_position(
        cls, chunk_pos: tuple, camera: camera.PseudoCamera
    ):
    ...
    def __getstate__(self) -> object:
        ...
        with open(
            WORLD_LEVEL_FOLDER
            + self._world_storage_key
            + "/"
            + WORLD_LEVEL_CHUNKS_FOLDER
            + _filename,
            "wb",
        ) as f:
            print(
                "Saving Chunk: ",
                self._chunk_hash_str,
                " | to file: ",
                WORLD_LEVEL_FOLDER
                + self._world_storage_key
                + "/"
    ...
    def __setstate__(self, state: object):
        ...
        self._tile_pixel_area = (
            (
                singleton.DEFAULT_TILE_WIDTH
                if not self.__tile_pixel_area[0]
                else self.__tile_pixel_area[0]
            ),
            (
                singleton.DEFAULT_TILE_HEIGHT
                if not self.__tile_pixel_area[1]
                else self.__tile_pixel_area[1]
            ),
        )
    ...
    def load_chunk_data(self):
        ...
        with open(
            ...
            _tiles = dill.load(f)
        self._tiles = [
            [None for x in range(len(_tiles[y]))] for y in range(len(_tiles))
        ]
...
class Layer:
    ...
    def __init__(self, layer_num: int) -> None:
        ...
        self._layer_buffer = pygame.Surface(
            singleton.FB_SIZE, singleton.DEFAULT_SURFACE_FLAGS, 16
        ).convert_alpha()
    ...
    def update(self, camera: camera.PseudoCamera):
        ...
        for chunk_hash_str in self._world._renderable_chunks_hash_strs:
            ...
            pygame.draw.rect(
                self._layer_buffer,
                (255, 255, 255, 150),
                Chunk.generate_chunk_rect_given_chunk_position(
                    self._chunks[chunk_hash_str]._chunk_position, camera
                ),
                1,
            )
    ...
    def __setstate__(self, state):
        ...
        self._layer_buffer = pygame.Surface(
            singleton.FB_SIZE, pygame.SRCALPHA, 16
        ).convert_alpha()
        self._layer_buffer.fill((0, 0, 0, 0))
...
class World:
    def __init__(self, name: str = None) -> None:
        ...
        if "New World" in os.listdir(WORLD_LEVEL_FOLDER):
            while (end_result_string := f"{__base} ({__num})") in os.listdir(
                WORLD_LEVEL_FOLDER
            ):
                __num += 1

        self._world_storage_key = (
            f"{name if name else ('New World' + end_result_string)}"
        )
        ...
        self._layers = {
            _ - 2: Layer(_ - 2) for _ in range(singleton.DEFAULT_LAYER_COUNT)
        }
    ...
    def iterate_renderable_chunk_positions(self):
        ...
        for cx in range(
            self._camera_old_chunk[0] - self._render_distance[0],
            self._camera_old_chunk[0] + self._render_distance[0] + 1,
        ):
            for cy in range(
                self._camera_old_chunk[1] - self._render_distance[0],
                self._camera_old_chunk[1] + self._render_distance[0] + 1,
            ):
                yield (cx, cy)
    ...
    def __getstate__(self):
        ...
        if not os.path.exists(WORLD_LEVEL_FOLDER + self._world_storage_key):
            print(
                "Creating world folder: ", WORLD_LEVEL_FOLDER + self._world_storage_key
            )
    ...
    def cache_world(cls, world):
        ...
        if world._world_storage_key in WORLD_CACHE:
            raise ValueError(
                f"World with key {world._world_storage_key} already exists in cache"
            )
    ...
    def load_world(cls, world_key: str):
        ...
        # load the world
        result = singleton.load_world(
            (WORLD_LEVEL_FOLDER if not world_key.startswith(WORLD_LEVEL_FOLDER) else "")
            + world_key
        )
